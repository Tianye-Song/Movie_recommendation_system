...
def get_statistic(records, history):
    ...
    for user, movies in history:
        ...
        total_recommendation_count += len(
            movies
        )  # Count how many movie recommendations.
        correctness = False
        for movie in list(records[user].keys()):
            if (
                movie in movies
            ):  # The user has watched a movie on the recommendation list.
                correctness = True
                num_correct += (
                    1  # Count how many recommended movies the user has watched.
                )
                total_rates += records[user][movie]  # Count the total rates.
                if movie == movies[0]:  # Count the top recommendation movie
                    top_rates += records[user][
                        movie
                    ]  # The rates of the top recommended movie.
                    top_count += (
                        1  # Count how any top recommended movies the user has watched.
                    )
        if correctness:
            num_correct_users += 1
...
def split_history(date):
    ...
    for userid, recommendation, model in history:
...
def ABtesting(date, timestamp):
    ...
    (
        ...
        top_count,
        avg_top_rates,
    ) = get_statistic(rating_records, history)
    temp = [
        num_correct,
        total_records_count,
        record_accuracy,
    ...
    print(
        "> Evaluatig finished Write to csv Eval"
        + timestamp
        + ".csv, time is "
...
def randomABtesting(date, timestamp, dir1, dir2):
    ...
    for userid in history:
        ...
        if int(user_id[0]) % 2 == 0:
            temp = (
                recommendation1[userid]
                if userid in recommendation1
                else recommendation1["NA"]
            )
            his1.append([userid, temp])
        else:
            temp = (
                recommendation2[userid]
                if userid in recommendation2
                else recommendation2["NA"]
            )
    ...
    (
        ...
        top_count,
        avg_top_rates,
    ) = get_statistic(rating_records, his1)
    temp = [
        num_correct,
        total_records_count,
        record_accuracy,
    ...
    (
        ...
        top_count,
        avg_top_rates,
    ) = get_statistic(rating_records, his2)
    temp = [
        num_correct,
        total_records_count,
        record_accuracy,
